...
printf "  ${WHITE}Skill:${RESET}   ~/.claude/skills/rick/ + Rick/ ${DIM}(Claude Code)${RESET}\n"
printf "           ~/.cursor/skills-cursor/rick/ ${DIM}(Cursor)${RESET}\n"
printf "  ${WHITE}Persona:${RESET} ~/.rick/persona/\n"
if [[ -n "$UNIVERSE_URL" ]]; then
    printf "  ${WHITE}Universe:${RESET} cloned and compiled\n"
fi
printf "\n"
printf "  ${DIM}Open Claude Code or Cursor and type /Rick to get started${RESET}\n"
printf "\n"
